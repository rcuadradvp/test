...
export class UIRenderer {
  constructor(elements) {
    ...
    this.TABLE_HEIGHT = '300px'; // Altura fija para ambas tablas
  }
  ...
  hideInitialSkeleton() {
    ...
    // Aplicar altura fija a los contenedores de las tablas
    this.applyTableHeight();
  }

  applyTableHeight() {
    // Buscar los contenedores de las tablas y aplicar altura fija
  ...
  updateTargetPreview(stateManager) {
    ...
    // Mostrar/ocultar preview solo cuando hay categoría seleccionada (no "unassigned")
  ...
  renderAvailableProducts(stateManager) {
    ...
    // Si hay departamento pero no categoría (excepto para "unassigned")
  ...
  attachRowClickEvents(panel, stateManager) {
    ...
    tbody.querySelectorAll('tr[data-product-id]').forEach(row => {
      row.addEventListener('click', (e) => {
        // No hacer nada si se clickeó en el checkbox
  ...
  updateCounters(stateManager) {
    ...
    // Transfer summary (botón principal)
  ...
  updateButtons(stateManager) {
    ...
    // Transfer button - requiere productos y destino
