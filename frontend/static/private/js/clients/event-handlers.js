// static/private/js/clients/event-handlers.js
...
export class EventHandlers {
  ...
  async loadData() {
    try {
      ...
      const data = await this.api.listClients();
      if (!data?.length) {
  ...
  async create(data) {
    try {
      const newItem = await this.api.createClient(data);
      const fullName = `${newItem.first_name || ''} ${newItem.last_name || ''}`.trim();
      MessageHelper?.success(`Cliente "${fullName}" creado exitosamente`);
      await this.loadData();
      return newItem;
    } catch (error) {
  ...
  async update(id, data) {
    try {
      const updatedItem = await this.api.updateClient(id, data);
      const fullName = `${updatedItem.first_name || ''} ${updatedItem.last_name || ''}`.trim();
      MessageHelper?.success(`Cliente "${fullName}" actualizado exitosamente`);
      await this.loadData();
      return updatedItem;
    } catch (error) {
  ...
  async confirmDelete(item) {
    try {
      await this.api.deleteClient(item.id);
      const fullName = `${item.first_name || ''} ${item.last_name || ''}`.trim();
      MessageHelper?.success(`Cliente "${fullName}" desactivado exitosamente`);
      await this.loadData();
    } catch (error) {
