// static/private/js/suppliers/modal-delete.js
...
export class DeleteModal {
  ...
  init(onConfirm) {
    this.modal = document.getElementById('modal-delete-supplier');
  ...
  setupEventListeners() {
    ...
    confirmBtn?.addEventListener('click', async () => {
      ...
      try {
        ...
        MessageHelper?.error(error.message || 'Error al desactivar proveedor');
      } finally {
        this.isDeleting = false;
  ...
  updateContent(item) {
    const nameElement = this.modal.querySelector('#delete-supplier-name');
    if (nameElement) {
      nameElement.textContent = item.name || 'este proveedor';
    }
  }
