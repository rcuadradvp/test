...
class App {
  constructor() {
    ...
    // Leer data attributes del HTML
    this.departmentSlug = this.elements.appContent?.dataset.departmentSlug || null;
    
    // Información del departamento (se cargará si es necesario)
    ...
    // EventHandlers se inicializará después de cargar info del departamento (si aplica)
  ...
  async init() {
    try {
      ...
      // Si tenemos department_slug, primero obtenemos la info del departamento
      if (this.departmentSlug) {
        await this.loadDepartmentInfo();
      }

      // Inicializar EventHandlers con el slug del departamento (o null)
  ...
  /**
   * Carga la información del departamento
   */
  ...
  /**
   * Actualiza la UI según el tipo de vista
   */
  updatePageUI() {
    ...
    if (this.departmentSlug && this.departmentInfo) {
      // Vista de productos sin categoría dentro de un departamento
      if (titleElement) {
      ...
      // Vista de productos sin departamento
      if (titleElement) {
